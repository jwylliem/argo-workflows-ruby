  ...
  class GithubComArgoprojLabsArgoDataflowApiV1alpha1Container
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ArgoWorkflows::GithubComArgoprojLabsArgoDataflowApiV1alpha1Container` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ArgoWorkflows::GithubComArgoprojLabsArgoDataflowApiV1alpha1Container`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [args, command, env, image, _in, resources, volume_mounts].hash
    end
