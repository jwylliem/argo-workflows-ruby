  ...
  class IoArgoprojEventsV1alpha1GitArtifact
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ArgoWorkflows::IoArgoprojEventsV1alpha1GitArtifact` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ArgoWorkflows::IoArgoprojEventsV1alpha1GitArtifact`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'clone_directory')
        self.clone_directory = attributes[:'clone_directory']
      end
      ...
      if attributes.key?(:'ssh_key_secret')
        self.ssh_key_secret = attributes[:'ssh_key_secret']
      end
    ...
    def hash
      [branch, clone_directory, creds, file_path, ref, remote, ssh_key_secret, tag, url].hash
    end
