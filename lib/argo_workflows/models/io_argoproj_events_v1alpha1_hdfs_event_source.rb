  ...
  class IoArgoprojEventsV1alpha1HDFSEventSource
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ArgoWorkflows::IoArgoprojEventsV1alpha1HDFSEventSource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ArgoWorkflows::IoArgoprojEventsV1alpha1HDFSEventSource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'watch_path_config')
        self.watch_path_config = attributes[:'watch_path_config']
      end
    end
    ...
    def hash
      [addresses, check_interval, hdfs_user, krb_c_cache_secret, krb_config_config_map, krb_keytab_secret, krb_realm, krb_service_principal_name, krb_username, metadata, type, watch_path_config].hash
    end
