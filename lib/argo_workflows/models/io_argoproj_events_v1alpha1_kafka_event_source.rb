  ...
  class IoArgoprojEventsV1alpha1KafkaEventSource
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ArgoWorkflows::IoArgoprojEventsV1alpha1KafkaEventSource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ArgoWorkflows::IoArgoprojEventsV1alpha1KafkaEventSource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'limit_events_per_second')
        self.limit_events_per_second = attributes[:'limit_events_per_second']
      end
    ...
    def hash
      [connection_backoff, consumer_group, json_body, limit_events_per_second, metadata, partition, sasl, tls, topic, url, version].hash
    end
