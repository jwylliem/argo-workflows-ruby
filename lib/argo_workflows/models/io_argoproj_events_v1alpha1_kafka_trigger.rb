  ...
  class IoArgoprojEventsV1alpha1KafkaTrigger
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ArgoWorkflows::IoArgoprojEventsV1alpha1KafkaTrigger` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ArgoWorkflows::IoArgoprojEventsV1alpha1KafkaTrigger`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'flush_frequency')
        self.flush_frequency = attributes[:'flush_frequency']
      end
      ...
      if attributes.key?(:'partitioning_key')
        self.partitioning_key = attributes[:'partitioning_key']
      end
    ...
    def hash
      [compress, flush_frequency, parameters, partition, partitioning_key, payload, required_acks, sasl, tls, topic, url, version].hash
    end
