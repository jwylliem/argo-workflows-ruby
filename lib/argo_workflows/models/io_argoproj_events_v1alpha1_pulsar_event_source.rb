  ...
  class IoArgoprojEventsV1alpha1PulsarEventSource
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ArgoWorkflows::IoArgoprojEventsV1alpha1PulsarEventSource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ArgoWorkflows::IoArgoprojEventsV1alpha1PulsarEventSource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'auth_token_secret')
        self.auth_token_secret = attributes[:'auth_token_secret']
      end

      if attributes.key?(:'connection_backoff')
        self.connection_backoff = attributes[:'connection_backoff']
      end
      ...
      if attributes.key?(:'tls_allow_insecure_connection')
        self.tls_allow_insecure_connection = attributes[:'tls_allow_insecure_connection']
      end

      if attributes.key?(:'tls_trust_certs_secret')
        self.tls_trust_certs_secret = attributes[:'tls_trust_certs_secret']
      end

      if attributes.key?(:'tls_validate_hostname')
        self.tls_validate_hostname = attributes[:'tls_validate_hostname']
      end
    ...
    def hash
      [auth_token_secret, connection_backoff, json_body, metadata, tls, tls_allow_insecure_connection, tls_trust_certs_secret, tls_validate_hostname, topics, type, url].hash
    end
