  ...
  class IoArgoprojWorkflowV1alpha1NodeStatus
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ArgoWorkflows::IoArgoprojWorkflowV1alpha1NodeStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ArgoWorkflows::IoArgoprojWorkflowV1alpha1NodeStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'boundary_id')
        self.boundary_id = attributes[:'boundary_id']
      end
      ...
      if attributes.key?(:'host_node_name')
        self.host_node_name = attributes[:'host_node_name']
      end
      ...
      if attributes.key?(:'memoization_status')
        self.memoization_status = attributes[:'memoization_status']
      end
      ...
      if attributes.key?(:'outbound_nodes')
        if (value = attributes[:'outbound_nodes']).is_a?(Array)
      ...
      if attributes.key?(:'synchronization_status')
        self.synchronization_status = attributes[:'synchronization_status']
      end
    ...
    def hash
      [boundary_id, children, daemoned, display_name, estimated_duration, finished_at, host_node_name, id, inputs, memoization_status, message, name, outbound_nodes, outputs, phase, pod_ip, progress, resources_duration, started_at, synchronization_status, template_name, template_ref, template_scope, type].hash
    end
