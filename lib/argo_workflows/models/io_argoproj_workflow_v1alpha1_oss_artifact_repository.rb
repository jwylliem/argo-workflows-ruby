  ...
  class IoArgoprojWorkflowV1alpha1OSSArtifactRepository
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ArgoWorkflows::IoArgoprojWorkflowV1alpha1OSSArtifactRepository` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ArgoWorkflows::IoArgoprojWorkflowV1alpha1OSSArtifactRepository`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'lifecycle_rule')
        self.lifecycle_rule = attributes[:'lifecycle_rule']
      end
    ...
    def hash
      [access_key_secret, bucket, create_bucket_if_not_present, endpoint, key_format, lifecycle_rule, secret_key_secret, security_token].hash
    end
