  ...
  class IoArgoprojWorkflowV1alpha1UserContainer
    ...
    def self.openapi_types
      {
        ...
        :'tty' => :'Boolean',
        :'volume_devices' => :'Array<VolumeDevice>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ArgoWorkflows::IoArgoprojWorkflowV1alpha1UserContainer` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ArgoWorkflows::IoArgoprojWorkflowV1alpha1UserContainer`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'mirror_volume_mounts')
        self.mirror_volume_mounts = attributes[:'mirror_volume_mounts']
      end
    ...
    def hash
      [args, command, env, env_from, image, image_pull_policy, lifecycle, liveness_probe, mirror_volume_mounts, name, ports, readiness_probe, resources, security_context, startup_probe, stdin, stdin_once, termination_message_path, termination_message_policy, tty, volume_devices, volume_mounts, working_dir].hash
    end
